...
struct Subscriber * addSbscr(struct Subscriber* temp,struct Topic *TList,char *topic, char *Sname)
{
        printf("topic exists\n");
...
struct SockAddr * addAddr(struct SockAddr *temp,char ip[20],int port,char *name)
{
    struct SockAddr *newnode = (struct SockAddr *)malloc(sizeof(struct SockAddr));
    if(temp==NULL)
    {
        temp = newnode;
...
struct SockAddr * GetMySock(struct SockAddr *List,char *myname)
{
    while(strcmp(List->Name,myname)!=0)
    {
        List = List->next;
    }
    return List;
}
void printAddr(struct SockAddr *temp)
{
    struct SockAddr *root = temp;
    while(temp->next!=root)
    {
        printf("%s at %s from %d\n",temp->Name,temp->IP,temp->port);
        temp = temp->next;
    }
        printf("%s at %s from %d\n",temp->Name,temp->IP,temp->port);
}
struct SockAddr* MakeAddress(struct SockAddr *temp)
{
    ...
    while(!feof(fp))
    {
        ...
        port=0;
        fscanf(fp,"%s %s %d\n",Name,IP,&port);
        temp = addAddr(temp,IP,port,Name);
    }
    return temp;
}
